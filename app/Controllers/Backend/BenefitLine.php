...
class BenefitLine extends BaseController
{
    public function __construct()
    {
        $this->request = Services::request();
        $this->model = new M_Benefit($this->request);
        $this->modelDetail = new M_BenefitLine($this->request);
    ...
    public function create()
    {
        if ($this->request->getMethod(true) === 'POST') {
            ...
            try {
                if ($this->isNew())
                    $this->entity->setBenefitId($post[$this->model->primaryKey]);

                if (!$this->validation->run($post, 'benefit_line')) {
    ...
    public function show($id = null)
    {
        if ($this->request->isAJAX()) {
            ...
            try {
                ...
                if (isset($get["md_benefit_id"])) {
                    $list = $this->model->where($this->model->primaryKey, $get["md_benefit_id"])->findAll();
                    $detail = $this->modelDetail->where($this->model->primaryKey, $get["md_benefit_id"])->findAll();
                }
    ...
    public function tableLine($set = null, $detail = [])
    {
        ...
        $mBenefitValue = new M_BenefitValue($this->request);
        ...
        if (!empty($set) && count($detail) > 0) {
            foreach ($detail as $row) :
                ...
                $count = $mBenefitValue->countAll($this->modelDetail->primaryKey, $id);
