...
class Bundling extends BaseController
{
    ...
    public function show($id)
    {
        ...
        $mMasterPacket = new M_MasterBundling($this->request);
        ...
        if ($this->request->isAJAX()) {
            try {
                ...
                $rowPacket = $mMasterPacket->where('name', $list[0]->name)->first();

                $list = $this->field->setDataSelect($mMasterPacket->table, $list, 'name', $rowPacket->name, $rowPacket->name);
