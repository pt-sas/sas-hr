...
class City extends BaseController
{
    public function __construct()
    {
        ...
        $this->model = new M_City($this->request);
        $this->entity = new \App\Entities\City();
    }

    public function index()
    {
        return $this->template->render('masterdata/city/v_city');
    }
    ...
    public function create()
    {
        if ($this->request->getMethod(true) === 'POST') {
            ...
            try {
                ...
                if (!$this->validation->run($post, 'city')) {
    ...
    {
        $province = new M_Province($this->request);

        if ($this->request->isAJAX()) {
            try {
                $list = $this->model->where($this->model->primaryKey, $id)->findAll();

                if (!empty($list[0]->getProvinceId())) {
                    $rowEmp = $province->find($list[0]->getProvinceId());
        ...
        if ($this->request->isAjax()) {
            ...
            try {
                if (isset($post['search'])) {
                    if (isset($post['md_province_id'])) {
                        $list = $this->model->where([
                            'isactive'      => 'Y',
                            'md_province_id' => $post['md_province_id']
                ...
                } else if (isset($post['md_province_id'])) {
                    $list = $this->model->where([
                        'isactive'      => 'Y',
                        'md_province_id' => $post['md_province_id']
                    ])->orderBy('name', 'ASC')
