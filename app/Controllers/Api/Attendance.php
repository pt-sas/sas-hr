...
class Attendance extends ApiController
{
    ...
    public function getInProgressSubmissions()
        {
            $status_code = null;

            try {
                $services = new AttendanceServices($this->jwt->sys_user_id, $this->jwt->md_employee_id);
                $result = $services->getInProgressSubmissions($this->jwt->md_employee_id);
                ...
                log_message('error', 'Attendance [getInProgressSubmissions] Error: ' . $e->getMessage() . ' | Line: ' . $e->getLine());
                $response = apiResponse(false, 'Internal Server Error');
                $status_code = 500;
            }
