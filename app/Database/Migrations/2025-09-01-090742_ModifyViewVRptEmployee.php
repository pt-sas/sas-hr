...
class ModifyViewVRptEmployee extends Migration
{
    public function up()
    {
        ...
        $this->db->query("CREATE VIEW v_rpt_employee AS 
                          (SELECT DISTINCT
                            ...
                            FROM ((((((((((((((((((((`md_employee` `e`
                                        ...
                                        ON ((`e`.`md_country_id` = `country`.`md_country_id`)))
                                    LEFT JOIN `md_province` `province`
                                        ON ((`e`.`md_province_id` = `province`.`md_province_id`)))
                                    LEFT JOIN `md_city` `city`
                                        ON ((`e`.`md_city_id` = `city`.`md_city_id`)))
                                    LEFT JOIN `md_district` `district`
                                    ON ((`e`.`md_district_id` = `district`.`md_district_id`)))
                                    LEFT JOIN `md_subdistrict` `subdistrict`
                                    ON ((`e`.`md_subdistrict_id` = `subdistrict`.`md_subdistrict_id`)))
