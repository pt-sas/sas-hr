...
class CreateViewReportEmpVaccine extends Migration
{
    public function up()
    {
        $this->db->query("CREATE VIEW v_rpt_employee_vaccine AS (SELECT e.`value` AS `Value`,
e.`fullname` AS `Nama Lengkap`,
branch.name AS `Cabang`,
divi.name AS `Divisi`,
ev.`vaccinetype` AS `Jenis Vaksin`,
DATE_FORMAT(ev.`vaccinedate` ,'%d-%m-%Y') AS `Tanggal Vaksin`,
ev.`description` AS `Keterangan`,
...
LEFT JOIN md_employee_vaccine ev ON e.`md_employee_id` = ev.`md_employee_id`
ORDER BY branch.md_branch_id, divi.md_division_id,e.md_employee_id
    ...
    {
        $this->db->query("DROP VIEW IF EXISTS v_rpt_employee_vaccine");
    }
}