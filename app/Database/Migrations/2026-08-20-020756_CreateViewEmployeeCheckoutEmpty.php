...
class CreateViewEmployeeCheckoutEmpty extends Migration
{
    public function up()
    {
        $this->db->query("CREATE VIEW v_employee_checkout_empty AS 
        WITH date_data as (SELECT add_workdays_backwards(date(now()), 1) AS tanggal)
        select me.md_employee_id, me.telegram_id, d.tanggal 
        from md_employee me
        ...
        LEFT JOIN md_employee_work mew_now ON me.md_employee_id = mew_now.md_employee_id and (mew_now.validfrom <= DATE(NOW()) and mew_now.validto >= DATE(NOW()))
        LEFT JOIN md_work_detail mwd_now ON mew_now.md_work_id = mwd_now.md_work_id AND (WEEKDAY(DATE(NOW())) + 1) = mwd_now.md_day_id
        ...
        and (CASE 
            WHEN mwd_now.md_work_detail_id IS NOT NULL AND HOUR(mwd_now.startwork) <> HOUR(NOW()) THEN FALSE
            ...
            WHEN mwd.md_work_detail_id IS NOT NULL THEN NOT EXISTS (SELECT 1 FROM v_attendance_branch vab 
                            ...
                            WHERE vab.md_employee_id = me.md_employee_id AND vab.`date` = d.tanggal AND (vab.clock_out IS NOT null or vab.clock_out != ''))
            ELSE false
        END)
        ...
        where a.submissiontype in (100001, 100003, 100004, 100005, 100007, 100009, 100011, 100013)
    ...
    {
        $this->db->query("DROP VIEW IF EXISTS v_employee_checkout_empty");
    }
}
