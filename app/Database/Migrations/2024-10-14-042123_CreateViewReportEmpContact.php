...
class CreateViewReportEmpContact extends Migration
{
    public function up()
    {
        $this->db->query("CREATE VIEW v_rpt_employee_contact AS (SELECT e.`value` AS `Value`,
e.`fullname` AS `Nama Lengkap`,
branch.name AS `Cabang`,
divi.name AS `Divisi`,
econ.`member` AS `Hubungan`,
econ.name AS `Nama`,
econ.`phone` AS `No HP`,
...
LEFT JOIN md_employee_contact econ ON e.`md_employee_id` = econ.`md_employee_id`
ORDER BY branch.md_branch_id, divi.md_division_id,e.md_employee_id
    ...
    {
        $this->db->query("DROP VIEW IF EXISTS v_rpt_employee_contact");
    }
}