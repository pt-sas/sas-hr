...
class CreateViewReportEmpCourses extends Migration
{
    public function up()
    {
        $this->db->query("CREATE VIEW v_rpt_employee_courses AS (SELECT e.`value` AS `Value`,
e.`fullname` AS `Nama Lengkap`,
branch.name AS `Cabang`,
divi.name AS `Divisi`,
ec.`course` AS `Kursus`,
ec.`intitution` AS `Tempat`,
ec.`level` AS `Level`,
DATE_FORMAT(ec.`startdate`,'%d-%m-%Y') AS `Tanggal Mulai`,
DATE_FORMAT(ec.`enddate`,'%d-%m-%Y') AS `Tanggal Selesai`,
ec.`status` AS `Status`,
...
LEFT JOIN md_employee_courses ec ON e.`md_employee_id` = ec.`md_employee_id`
ORDER BY branch.md_branch_id, divi.md_division_id,e.md_employee_id
    ...
    {
        $this->db->query("DROP VIEW IF EXISTS v_rpt_employee_courses");
    }
}