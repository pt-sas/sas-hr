...
class CreateViewReportEmpSkillsLanguage extends Migration
{
    public function up()
    {
        $this->db->query("CREATE VIEW v_rpt_employee_language AS (SELECT e.`value` AS `Value`,
e.`fullname` AS `Nama Lengkap`,
branch.name AS `Cabang`,
divi.name AS `Divisi`,
skill.name AS `Penguasaan Bahasa`,
skill.written_ability AS `Penguasaan Tertulis`,
skill.verbal_ability AS `Penguasaan Lisan`,
...
ORDER BY branch.md_branch_id, divi.md_division_id,e.md_employee_id DESC
    ...
    {
        $this->db->query("DROP VIEW IF EXISTS v_rpt_employee_language");
    }
}