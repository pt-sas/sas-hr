...
class CreateViewReportEmpFamily extends Migration
{
    public function up()
    {
        $this->db->query("CREATE VIEW v_rpt_employee_family AS (SELECT DISTINCT e.`value` AS `Value`,
e.`nik` AS `NIK`,
...
LEFT JOIN md_employee_family famcore ON e.`md_employee_id` = famcore.`md_employee_id`
ORDER BY branch.md_branch_id, divi.md_division_id,e.md_employee_id, famcore.member DESC
    ...
    {
        $this->db->query("DROP VIEW IF EXISTS v_rpt_employee_family");
    }
}